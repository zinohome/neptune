...
bind = '#NEPTUNE_gunicorn_ip#:#NEPTUNE_gunicorn_port#'      # 绑定ip和端口号
backlog = 512                # 监听队列
chdir = '/opt/neptun'  # gunicorn要切换到的目的工作目录
timeout = #NEPTUNE_gunicorn_timeout#      # 超时
worker_class = 'uvicorn.workers.UvicornWorker'  # 使用gevent模式，还可以使用sync 模式，默认的是sync模式
# workers = multiprocessing.cpu_count() * 2 + 1    # 进程数
workers = #NEPTUNE_gunicorn_workers#
threads = #NEPTUNE_gunicorn_threads#  # 指定每个进程开启的线程数
